...
def stable_rank(singular_vals):
    s2_sum = torch.sum(singular_vals)
    smax2 = singular_vals[0] + 1e-12
    return (s2_sum / smax2).item()
...
def sampling_results_to_QA(results, idx2item,args):
    ...
    for item in results:
        ...
        if item["answer_em"]>= args.acc_threshold:
            ...
            # if len(set(new_item["pred_answer"])) == 1:
            #     continue
        else:
            new_item["pred_answer"] = [
                                    s["pred"]
...
def find_all_mlp_linear_names(model):
    # selected_layers = [4, 8, 12, 16, 20, 24, 30]
    cls = torch.nn.Linear
...
def compute_effective_condition_number(singular_vals, eps=1e-12):
    ...
    lambda_max = singular_vals[0]
    lambda_min = singular_vals[-1]

    return (lambda_max / (lambda_min + eps)).item()
...
def get_gradients(dataloader, model, collector, target_modules, gradient_type,args):
    ...
    rank_dir = os.path.join(
        base_dir,
        f"g_rank_sampling_{args.delta}delta_{args.base_name}_gen",
        gradient_type
    )
    sim_dir = os.path.join(
        base_dir,
        f"alignment_sampling_{args.delta}delta_{args.base_name}_gen",
        gradient_type
    )
...
def split_sampling_results_into_4_groups(
    ...
    for item in all_results:
        ...
        if acc >= acc_threshold:
            ...
            if args.data_name == "tqa" and len(set(preds)) <= 1:
                continue
        else:
            continue
...
def main():
    ...
    for gradient_type, results in groups:
        ...
        print(len(data_lis))
        dataset = GetGradientDataset_gen(
